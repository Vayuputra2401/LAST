...
No internal residual — x_back is returned directly.
The outer ShiftFuseBlock residual (res + out) handles gradient flow.
freq_mask init=+4.0 → sigmoid(4.0)≈0.982 ≈ near-identity at init.
...
class FrozenDCTGate(nn.Module):
    ...
    def __init__(self, channels: int, T: int):
        ...
        # Init +4.0 → sigmoid(4.0) ≈ 0.982 → x_back ≈ 0.982x (near-identity).
        # No internal residual: outer ShiftFuseBlock res+out handles identity path.
        # Model learns to filter frequencies by decreasing logits below +4.0.
        # Excluded from weight decay (see trainer.py no_decay list).
        self.freq_mask = nn.Parameter(torch.full((1, channels, T, 1), 4.0))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: (B, C, T, V)
        Returns:
            out: (B, C, T, V) — frequency-filtered version of x (no internal residual)
        """
        ...
        # No internal residual — outer block residual (res+out) handles identity path
        return x_back
