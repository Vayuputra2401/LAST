...
class SGPShift(nn.Module):
    ...
    def __init__(
        ...
        # Build per-joint target-joint indices for each group.
        # For each joint v: choose the most-connected neighbor in A;
        # if no neighbor exists (isolated joint), fall back to self.
        intra_idx = self._build_shift_idx(A_intra, num_joints)  # (V,)
        inter_idx = self._build_shift_idx(A_inter, num_joints)  # (V,)
        identity  = torch.arange(num_joints)                     # (V,)

        # Assemble full (C, V) index tensor:
        #   rows 0..g0-1       → intra_idx (broadcast)
        #   rows g0..g0+g1-1   → inter_idx (broadcast)
        #   rows g0+g1..C-1    → identity  (broadcast)
        idx = torch.cat([
            intra_idx.unsqueeze(0).expand(g0, -1),
            inter_idx.unsqueeze(0).expand(g1, -1),
            identity.unsqueeze(0).expand(channels - g0 - g1, -1),
        ], dim=0)  # (C, V)
