...
\node[free, below=0.15cm of brasp]  (sgp)   {SGPShift};
\node[learn, below=0.15cm of sgp]   (je)    {JointEmbed};
\node[learn, below=0.15cm of je]    (gcn)   {Graph Conv};
\node[learn, below=0.15cm of gcn]   (tcn)   {DS-TCN};
\node[free, below=0.15cm of tcn]    (dp)    {DropPath};
\node[learn, below=0.15cm of dp]    (res)   {$\oplus$ Skip};
...
%% TLA
\node[learn, fill=blue!15, below=0.4cm of res] (tla) {TLA (last)};
\draw[arr] (res)--(tla);

%% classifier
\node[cls, minimum height=2.8cm, minimum width=1.5cm]
  (pool) at (7.5, -3.20) {Gated\\Pool\\+ FC};
...
%% legend
\node[free,  minimum width=1.1cm] at (0,-5.50) {0-param};
\node[learn, minimum width=1.1cm] at (1.55,-5.50) {learnable};
\node[stem,  minimum width=1.1cm] at (3.1,-5.50) {fusion};
...
\node[free, below=0.20cm of brasp] (sgp)   {SGPShift\ \ (Eq.\,2)};
\node[learn, below=0.20cm of sgp]  (je)    {Joint Embedding $\mathbf{b}_v$};
\node[learn, below=0.20cm of je]   (stca)  {STC-Attention ($V{\times}V$)};
\node[learn, below=0.20cm of stca] (gcn)   {Graph Conv $K{=}3 + \mathbf{A}_\ell$\ \ (Eq.\,3)};
\node[learn, below=0.20cm of gcn]  (tcn)   {DS-TCN\ (dil.\,1\,+\,dil.\,2)};
\node[free, below=0.20cm of tcn]   (dp)    {DropPath};
\node[learn, below=0.20cm of dp]   (res)   {Residual $\oplus$};
\node[hilit, below=0.20cm of res]  (tla)   {TLA\ \ (last block only)\ \ (Eq.\,4)};
...
%% annotations right
\node[ann, right=0.15cm of brasp] {0 params, 0 FLOPs};
\node[ann, right=0.15cm of sgp]   {0 params, sparse avg};
\node[ann, right=0.15cm of je]    {$C_\text{in}{\times}V$ params};
\node[ann, right=0.15cm of gcn]   {$3C^2$ params};
\node[ann, right=0.15cm of tcn]   {$\approx C^2/2$ params};
\node[ann, right=0.15cm of dp]    {0 params};
\node[ann, right=0.15cm of tla]   {$O(T{\cdot}K)$ complexity};
...
\node[stem] (sm_sfA) at (4.0,-1.00) {Stem A\\J+B};
\node[stem] (sm_sfB) at (6.0,-1.00) {Stem B\\V+BV};
\node[blk]  (sm_12a) at (4.0,-2.00) {S1-S2 $(1{+}2)$\\$C{=}32{\to}64$};
\node[blk]  (sm_12b) at (6.0,-2.00) {S1-S2 $(1{+}2)$\\$C{=}32{\to}64$};
\node[fus]  (sm_csf) at (5.0,-3.10) {Cross-Stream\\Fusion (Eq.\,5)};
\node[tlabox](sm_s3a) at (4.0,-4.20){S3 $\times$1 + TLA\\$C{=}128$};
\node[tlabox](sm_s3b) at (6.0,-4.20){S3 $\times$1 + TLA\\$C{=}128$};
\node[cls]  (sm_cl)  at (5.0,-5.20) {Weighted avg + FC(60)};
