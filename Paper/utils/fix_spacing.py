...
\node[free, below=0.35cm of brasp]  (sgp)   {SGPShift};
\node[learn, below=0.35cm of sgp]   (je)    {JointEmbed};
\node[learn, below=0.35cm of je]    (gcn)   {Graph Conv};
\node[learn, below=0.35cm of gcn]   (tcn)   {DS-TCN};
\node[free, below=0.35cm of tcn]    (dp)    {DropPath};
\node[learn, below=0.35cm of dp]    (res)   {$\oplus$ Skip};
...
\node[font=\tiny, text=learnblue!80] at (5.0, 0.95) {EfficientZero Block $\times N$};

%% TLA
\node[learn, fill=blue!15, below=0.55cm of res] (tla) {TLA (last)};
\draw[arr] (res)--(tla);
...
%% legend
\node[free,  minimum width=1.1cm] at (0,-6.50) {0-param};
\node[learn, minimum width=1.1cm] at (1.55,-6.50) {learnable};
\node[stem,  minimum width=1.1cm] at (3.1,-6.50) {fusion};
...
\node[free, below=0.45cm of brasp] (sgp)   {SGPShift\ \ (Eq.\,2)};
\node[learn, below=0.45cm of sgp]  (je)    {Joint Embedding $\mathbf{b}_v$};
\node[learn, below=0.45cm of je]   (stca)  {STC-Attention ($V{\times}V$)};
\node[learn, below=0.45cm of stca] (gcn)   {Graph Conv $K{=}3 + \mathbf{A}_\ell$\ \ (Eq.\,3)};
\node[learn, below=0.45cm of gcn]  (tcn)   {DS-TCN\ (dil.\,1\,+\,dil.\,2)};
\node[free, below=0.45cm of tcn]   (dp)    {DropPath};
\node[learn, below=0.45cm of dp]   (res)   {Residual $\oplus$};
\node[hilit, below=0.45cm of res]  (tla)   {TLA\ \ (last block only)\ \ (Eq.\,4)};
...
%% annotations right
\node[ann, right=0.25cm of brasp] {0 params, 0 FLOPs};
\node[ann, right=0.25cm of sgp]   {0 params, sparse avg};
\node[ann, right=0.25cm of je]    {$C_\text{in}{\times}V$ params};
\node[ann, right=0.25cm of gcn]   {$3C^2$ params};
\node[ann, right=0.25cm of tcn]   {$\approx C^2/2$ params};
\node[ann, right=0.25cm of dp]    {0 params};
\node[ann, right=0.25cm of tla]   {$O(T{\cdot}K)$ complexity};
...
\node[stem] (sm_sfA) at (3.5,-1.00) {Stem A\\J+B};
\node[stem] (sm_sfB) at (6.5,-1.00) {Stem B\\V+BV};
\node[blk]  (sm_12a) at (3.5,-2.10) {S1-S2 $(1{+}2)$\\$C{=}32{\to}64$};
\node[blk]  (sm_12b) at (6.5,-2.10) {S1-S2 $(1{+}2)$\\$C{=}32{\to}64$};
\node[fus]  (sm_csf) at (5.0,-3.30) {Cross-Stream\\Fusion (Eq.\,5)};
\node[tlabox](sm_s3a) at (3.5,-4.50){S3 $\times$1 + TLA\\$C{=}128$};
\node[tlabox](sm_s3b) at (6.5,-4.50){S3 $\times$1 + TLA\\$C{=}128$};
\node[cls]  (sm_cl)  at (5.0,-5.60) {Weighted avg + FC(60)};
...
\node[stem] (lg_sfJ) at ( 8.5,-0.85) {Stem J};
\node[stem] (lg_sfB) at (11.0,-0.85) {Stem B};
\node[stem] (lg_sfV) at (13.5,-0.85) {Stem V};
\node[blk]  (lg_12J) at ( 8.5,-1.95) {S1-S2 $(1{+}2)$\\$C{=}48{\to}96$};
\node[blk]  (lg_12B) at (11.0,-1.95) {S1-S2 $(1{+}2)$\\$C{=}48{\to}96$};
\node[blk]  (lg_12V) at (13.5,-1.95) {S1-S2 $(1{+}2)$\\$C{=}48{\to}96$};
...
\node[cls]   (lg_cl) at (11.0,-5.50) {Pool-Gate + FC(60)};
